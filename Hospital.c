...
void insertEnd(LL *l,int pnum,char pname[30],int page,char pdisease[50],int pbedno, char doctor_name[50], int phone)
{
    node *newrec,*p;
	...
	newrec->bedno=pbedno;
	strcpy(newrec->doctor_name, doctor_name);
	newrec->pphone = phone;
...
void display(LL *l)
{
    ...
    printf("\tSR.NO.\t\tID\t\tNAME\t\tAGE\t\tDISEASE\t\tBED NO\t\tDOCTOR NAME\tPHONE NUMBER\n");

	while(p!=NULL)//Traversing through the list
	{
        i++;
        printf("\t%d\t\t%d\t\t%s\t\t%d\t\t%s\t\t%d\t\t%s\t\t0%d\n",i,p->num,p->name,p->age,p->disease,p->bedno, p->doctor_name, p->pphone);//printing the records in the table form
		p=p->next;//go to next node
	}
	printf("\n");
}
...
void search(LL *l,char pname[30])
{
	...
	while(p!=NULL)//Traversing the LL
	{
        ...
        {
            printf("\n");
            printf("\t\tID\t\tNAME\t\tAGE\t\tDISEASE\t\tBED NO\t\tDOCTOR NAME\tPHONE NUMBER\n");
            printf("\t\t%d\t\t%s\t\t%d\t\t%s\t\t%d\t\t%s\t\t0%d\n",p->num,p->name,p->age,p->disease,p->bedno, p->doctor_name, p->pphone);
            break;
		}
		else
		{
...
int main()
{
	...
	LL l1;
	int ch,pnum,page,pbedno,phone;
	char pname[30],pdisease[50], doctor_name[50],d,c,b;
    l1.start=NULL;
    printf("*******************************************************************************************************\n*******************************************||BUBT HOSPITAL||*******************************************\n*******************************************************************************************************\n");
	printf("\n\t MENU:\n");
    printf("\n\t1. INSERT RECORD\n");
	...
	while(1)
	{
		...
		switch(ch)
		{
            case 1:
                    ...
                    insertEnd(&l1,pnum,pname,page,pdisease,pbedno, doctor_name, phone);
